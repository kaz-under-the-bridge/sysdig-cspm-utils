...
func TestGetInventoryResources(t *testing.T) {
	server := testutil.NewMockServer(testutil.DefaultMockServerConfig())
	defer server.Close()
	...
	tests := []struct {
		...
		{
			name:      "Security Groupフィルター",
			filter:    `control.failed in ("Networking - Disallowed Public Access")`,
			limit:     10,
			wantError: false,
			checkData: true,
		},
		{
			name:      "S3 Bucketフィルター",
			filter:    `control.failed in ("Storage - S3 Bucket Public Read")`,
			limit:     10,
			wantError: false,
			checkData: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			response, err := client.GetInventoryResources(tt.filter, tt.limit)
...
func TestGetInventoryResourcesPaginated(t *testing.T) {
	server := testutil.NewMockServer(testutil.DefaultMockServerConfig())
	defer server.Close()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			response, err := client.GetInventoryResourcesPaginated(tt.filter, tt.pageNumber, tt.pageSize)
...
func TestGetAllInventoryResources(t *testing.T) {
	server := testutil.NewMockServer(testutil.DefaultMockServerConfig())
	defer server.Close()
	...
	// フィルター付きでテスト（モックサーバーが対応しているフィルター）
	response, err := client.GetAllInventoryResources(`control.failed in ("Networking - Disallowed Public Access")`, 2, 1, 0)
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
		return
	}
